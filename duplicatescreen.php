...
$query1 ="insert into DiscountFocCustomer(focheaderid,cuscode) select '$rand',cuscode from DiscountFocCustomer where focheaderid='$focid'";
$stmt = sqlsrv_query( $con, $query1);   
  
$query2="insert into DiscountFocProduct(focheaderid,procode,originalprice,discountprice,discountper) select '$rand',procode,originalprice,discountprice,discountper from DiscountFocProduct where focheaderid='$focid'";
$stmt = sqlsrv_query( $con, $query2);	

$date = date('Y-m-d');


    echo '<div class="container-fluid">
  <h3 style="text-align:center;"><u>Duplicate Discount</u></h3>
  <br>
...
<div class="row">
  ...
  echo'   <select style="width:20%;margin-left:15px;" class="form-control" id="pname" name="pname" onchange="getpid(this.value)">
       ...
       echo' <option class="form-control" value='.$row['id'].'>'.$row['pname'].'</option>';
       }
      echo '    
          </select> ';
...
<div id="results"></div>
   ...
   <div class="table-responsive"style="padding-top:30px;">
			  ...
			  on p.procode=i.Code where p.focheaderid='$focid'";
   ...
   <div class="row"> 
	...
	echo '
     <select style="width:50%;margin-left:60px;" class="form-control" id="cusname" name="cusname" onchange="getcusid(this.value)">
       ...
       echo' <option class="form-control" value='.$row['id'].'>'.$row['cusname'].'</option>';
       }
	   echo '
          </select> 
      </div>
...
<label style="margin-left:10px"></label>
      ...
      <div class="table-responsive"style="padding-top:80px;">
                    <table class="table table-bordered" >
						  ...
						  DiscountFocCustomer c inner join customer i on c.cuscode=i.Code where c.focheaderid='$focid'"; 
...
$.ajax({
                    ...
                    success:function(){
                        alert("Record Inserted Successfully");
						 window.open("viewscreen.php?id="+codeafter, "_self");
